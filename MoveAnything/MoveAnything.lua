...
--X: hook replacements
	...
	hookArenaEnemyFrames = function()
		if ArenaPrepFrames and not ArenaPrepFrames.hooked_ma then
				...
				ArenaPrepFrames.ClearAllPoints = function(self) if not InCombatLockdown() then self:clear_all_points() end end
				ArenaPrepFrames.set_points = ArenaPrepFrames.SetPoint
				ArenaPrepFrames.SetPoint = function(self, ...) if not InCombatLockdown() then self:set_points(...) end end
			end
			...
			if ArenaEnemyFrames and not ArenaEnemyFrames.hooked_ma then
				...
				ArenaEnemyFrames.ClearAllPoints = function(self) if not InCombatLockdown() then self:clear_all_points() end end
				ArenaEnemyFrames.set_points = ArenaEnemyFrames.SetPoint
				ArenaEnemyFrames.SetPoint = function(self, ...) if not InCombatLockdown() then self:set_points(...) end end
			end
	end,
	...
	hookArenaEnemyFrames15 = function(self)
			...
			if MovAny:IsModified("ArenaEnemyFrame1") then 
			...
			if MovAny:IsModified("ArenaPrepFrame1") then 
...
--	end
	...
	--C_StorePublic.IsDisabledByParentalControls = function() 
	--	print("Hello Blizzrd from MoveAnything. PLEASE FIRE YOUR CODERS ALREADY. C_StorePublic.IsDisabledByParentalControls Removed THX YOU") 
--	end
...
function MovAny:MoverUpdatePosition(mover)
	...
	if mover.tagged  then
		...
		local fn = f:GetName()
		if fn == "FramerateLabel" then
			p =  self:GetRelativePoint({"BOTTOMLEFT", "UIParent", "BOTTOMLEFT"}, f)
		elseif fn =="WorldStateAlwaysUpFrame" then
			p =  self:GetRelativePoint({"BOTTOMLEFT", "UIParent", "BOTTOMLEFT"}, f)
			
			local m = 4.3750000977888400763751782264927 * f:GetEffectiveScale()
			p[4] = p[4] + m
			p[5] = p[5] + m
		else
...
--	assert((num < 8000), "Too much childrens stuck in owerflow")
	...
	if size > 0 then
		...
		for i, v in pairs(relatives) do
			if v:GetName() ~= nil and not self:IsContainer(v:GetName()) and not string.match(v:GetName(), "BagFrame[1-9][0-9]*") and not self.NoUnanchoring[ v:GetName() ] and not v.MAPoint then -- alternatively use not self:GetUserData(v:GetName()) instead of v.MAPoint
...
--[[
			if i == "cat" then
				maPrint("  category: "..v.displayName)
			elseif i ~= "name" then
			]]
...
function MovAny_OnEvent(self, event, arg1)
	...
	elseif event == "ADDON_LOADED" then
		...
		--	hooksecurefunc("ArenaEnemyFrames_UpdateWatchFrame", MovAny.hWatchFrameExpand)
			...
			--	ArenaPrepFrames.Show = function(self) if not InCombatLockdown() then self:ma_Show() end end
				...
				ArenaPrepFrames.IsShown = function()
					...
					if ( instanceType == "arena" ) or ( instanceType == "pvp" ) then
						...
						if ArenaPrepFrame1:IsShown() then return true end
						if ArenaPrepFrame2:IsShown() then return true end
						if ArenaPrepFrame3:IsShown() then return true end
						if ArenaPrepFrame4:IsShown() then return true end
						if ArenaPrepFrame5:IsShown() then return true end					
					end
					return false
				end
			...
			--	ArenaPrepFrames.Hide = function(self) if not InCombatLockdown() then self:ma_Hide() end end
			...
			--	ArenaEnemyFrames.Show = function(self) if not InCombatLockdown() then self:ma_Show() end end
				...
				ArenaEnemyFrames.IsShown = function()
					...
					if ( instanceType == "arena" ) or ( instanceType == "pvp" ) then
						...
						if ArenaEnemyFrame1:IsShown() then return true end
						if ArenaEnemyFrame2:IsShown() then return true end
						if ArenaEnemyFrame3:IsShown() then return true end
						if ArenaEnemyFrame4:IsShown() then return true end
						if ArenaEnemyFrame5:IsShown() then return true end					
					end
					return false
				end
				
				ArenaEnemyFrames.ma_Hide = ArenaEnemyFrames.Hide
			--	ArenaEnemyFrames.Hide = function(self) if not InCombatLockdown() then self:ma_Hide() end end
